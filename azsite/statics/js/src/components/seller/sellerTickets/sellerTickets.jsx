...
import SellerTicketsFilterBar from './sellerTicketsFilterBar';
import SellerTicketsCard from './sellerTicketsCard';
import SellerTicketsEmpty from './sellerTicketsEmpty';
import SellerTicketsSkeleton from './sellerTicketsSkeleton';
import SellerTicketsDetailModal from './sellerTicketsDetailModal';
...
const SellerTickets = () => {
    ...
    useEffect(() => {
        const load = async () => {
            ...
            const sample = Array.from({ length: 10 }, (_, i) => ({
                ...
                messages: [
                    { sender: 'user', text: 'سلام، من مشکل دارم با سفارشم. لطفاً راهنمایی کنید.', date: new Date(Date.now() - i * 2 * 86400000).toLocaleDateString('fa-IR') },
                    { sender: 'seller', text: 'سلام، لطفاً شماره سفارش خود را ارسال کنید.', date: new Date(Date.now() - (i * 2 - 1) * 86400000).toLocaleDateString('fa-IR') },
                ],
            }));
            setTickets(sample);
            setIsLoading(false);
    ...
    const fuse = useMemo(() => new Fuse(tickets, { keys: ['ticketCode', 'subject', 'customer'], threshold: 0.3 }), [tickets]);
    ...
    const handleReply = (id, text) => {
        setTickets(prev => prev.map(t => t.id === id ? { ...t, messages: [...t.messages, { sender: 'seller', text, date: new Date().toLocaleDateString('fa-IR') }], lastUpdate: new Date().toLocaleDateString('fa-IR'), status: 'answered' } : t));
    ...
    if (isLoading) return <SellerTicketsSkeleton />;
    ...
    return (
        <div className="space-y-5">
            ...
            {filtered.length === 0 ? (
                <SellerTicketsEmpty hasFilter={filterStatus !== 'all' || !!searchQuery} />
            ) : (
                <>
                    <div className="space-y-3">
                        {paginated.map(ticket => (
                            <SellerTicketsCard key={ticket.id} ticket={ticket} statusMap={statusMap} onView={() => setSelectedTicket(ticket)} />
                        ))}
                    </div>
            ...
            {selectedTicket && (
                <SellerTicketsDetailModal
                    ...
                    onCloseTicket={() => handleClose(selectedTicket.id)}
                />
            )}
        </div>
    );
