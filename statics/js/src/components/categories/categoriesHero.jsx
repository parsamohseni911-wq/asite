// =============================================================================
// FILE: categoriesHero.jsx
// =============================================================================
import React, { useState } from 'react';
import { Search, X, Grid } from 'react-feather';

const CategoriesHero = ({ totalCategories = 0, searchQuery = '', onSearch, onClear }) => {
    const [inputValue, setInputValue] = useState(searchQuery);

    const handleSubmit = () => onSearch(inputValue);
    const handleKeyDown = (e) => { if (e.key === 'Enter') handleSubmit(); };
    const handleClear = () => { setInputValue(''); onClear(); };
                    ...
                    <div className="flex items-center gap-3 mb-1">
                        ...
                        <div>
                            <h1 className="text-2xl sm:text-3xl font-extrabold text-white">
                                دسته‌بندی <span className="text-amber-400">محصولات</span>
                            </h1>
                            <p className="text-sm text-white/60 mt-0.5">
                                {totalCategories.toLocaleString('fa-IR')} دسته‌بندی متنوع
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex-1 w-full sm:max-w-md sm:mr-auto">
                    <div className="relative">
                        ...
                        <div className="absolute left-1.5 top-1/2 -translate-y-1/2 flex items-center gap-1">
                            ...
                            <button onClick={handleSubmit} className="px-3 py-1.5 bg-amber-400 text-[#002874] rounded-lg text-xs font-bold hover:bg-amber-300 transition-colors">
                                جستجو
                            </button>
                        </div>
                    </div>
