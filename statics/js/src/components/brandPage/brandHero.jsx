...
import { Award, Package, Search, X } from 'react-feather';
import { LazyLoadImage } from 'react-lazy-load-image-component';

const BrandHero = ({ brand, productsCount, searchInput, onSearchChange, onSearch, onClearSearch }) => (
    <div className="relative mt-3 overflow-hidden rounded-2xl bg-gradient-to-br from-[#002874] via-[#003399] to-[#1a1a5e] p-5 sm:p-7 lg:p-9 mb-5">
        ...
        <div className="relative z-10 flex flex-col sm:flex-row items-start sm:items-center gap-4">
            <div className="flex items-center gap-3">
                <div className="w-16 h-16 rounded-2xl bg-white/20 backdrop-blur p-2 flex items-center justify-center overflow-hidden">
                    {brand.logo ? <LazyLoadImage src={brand.logo} alt={brand.name} effect="blur" className="w-full h-full object-contain" /> : <Award size={28} className="text-white" />}
                </div>
                <div>
                    <h1 className="text-2xl sm:text-3xl font-extrabold text-white">{brand.name}</h1>
