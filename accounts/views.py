...
def verify_phone(request):
    ...
    # اگر OTP قبلی هنوز معتبر است دوباره ارسال نکن
    if (
        not created and
        (now - otp.created_at).total_seconds()
        < settings.OTP_RESEND_TIME
    ):

        remaining = settings.OTP_EXPIRE_TIME - int(
            (now - otp.created_at).total_seconds()
        )
...
def forgot_password_verify(request):
    ...
    # OTP صحیح است
    request.session["password_reset_account"] = account.id

    # OTP را حذف می‌کنیم تا دوباره قابل استفاده نباشد
    otp.delete()
